...
namespace GameTab {
    ...
    void Render() {
        ...
        if (openGeneral) {
            ...
            if (IsHost() || !State.SafeMode) (ToggleButton("No Ability Cooldown", &State.NoAbilityCD)); {
                State.Save();
            }
            ...
            if ((IsInGame() || IsInLobby())) {
                ImGui::SameLine();
                if (ImGui::Button("Protect Friends")) { //Protect friends
                        if (IsInGame())
                            for (auto p : GetAllPlayerControl()) {
                                if (p != *Game::pLocalPlayer) State.RpcQueue.push(new RpcProtectPlayer(*Game::pLocalPlayer, p, 1, true));
                                if (!State.Friends.contains(convert_from_string(p->fields.Puid))) continue; // only friends
                                else if (IsInLobby())
                                    if (p != *Game::pLocalPlayer) State.lobbyRpcQueue.push(new RpcProtectPlayer(*Game::pLocalPlayer, p, 1, true)); ----------------> Unfinished [Luckyheat]
                                if (!State.Friends.contains(convert_from_string(p->fields.Puid))) continue; // only friends
                        }
                    }
                }
            ...
            ImGui::Dummy(ImVec2(5, 5)* State.dpiScale);
            if ((IsHost() || !State.SafeMode) && ImGui::Button("Allow Everyone to NoClip")) {
                for (auto p : GetAllPlayerControl()) {
            ...
            if ((IsHost() || !State.SafeMode) && ImGui::Button("Allow Friends to NoClip"))  {
                for (auto p : GetAllPlayerControl()) {
                    if (!State.Friends.contains(convert_from_string(p->fields.Puid))) continue; // only friends
                    if (p != *Game::pLocalPlayer) State.lobbyRpcQueue.push(new RpcMurderLoop(*Game::pLocalPlayer, p, 1, true));
                    ShowHudNotification("Allowed friends to NoClip!");
                }
            }


            ImGui::Dummy(ImVec2(5, 0) * State.dpiScale);
            if (IsInGame() || IsInLobby()) {
