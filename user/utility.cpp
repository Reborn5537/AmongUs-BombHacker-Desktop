...
void ShowHudNotification(std::string text) {
	std::string notificationText = "</size><#fb0>[<#0f0>Bomb</color><#f00>Hacker</color>]</color> " + text + "<size=0>";
	if (IsInGame() || IsInLobby())
...
std::vector<std::string> GetAllConfigs() {
	...
	auto configsPath = path.parent_path() / "bombhacker-config";
...
bool CheckConfigExists(std::string configName) {
	auto path = getModulePath(hModule);
	auto settingsPath = path.parent_path() / std::format("bombhacker-config/{}.json", State.selectedConfig);
...
void SMAC_OnCheatDetected(PlayerControl* pCtrl, std::string reason) {
	...
	if (IsHost()) {
		...
		case 2:
		{
			String* newName = convert_to_string(name + " has been kicked by <#0f0>Bomb</color><#f00>Hacker</color> <#9ef>Anticheat</color>! Reason: " + reason + "<size=0>");
		...
		case 3:
		{
			String* newName = convert_to_string(name + " has been banned by <#0f0>Bomb</color><#f00>Hacker</color> <#9ef>Anticheat</color>! Reason: " + reason + "<size=0>");
			PlayerControl_CmdCheckName(pCtrl, newName, NULL);
