...
bool GameVersionCheck() {
	...
	if (!IsWindows10OrGreater()) {
		...
		MessageBox(NULL, L"This version of Windows is not supported!", L"BombHacker", MB_OK | MB_ICONERROR | MB_SYSTEMMODAL);
		return false;
	}

	if (!std::filesystem::exists(gameAssembly)) {
		Log.Error("GameAssembly.dll was not found");
		MessageBox(NULL, L"Unable to locate GameAssembly.dll", L"BombHacker", MB_OK | MB_ICONERROR | MB_SYSTEMMODAL);
		return false;
	}
...
void Run(LPVOID lpParam) {
	...
	{
		std::ostringstream ss;
		ss << "\n\tBombHacker - " << __DATE__ << " - " << __TIME__ << std::endl; // Log SickoMenu info
		...
		LOG_INFO(ss.str());
#if _DEBUG
		SetConsoleTitleA(std::format("Debug Console - BombHacker {} (Among Us v{})", State.SickoVersion, getGameVersion()).c_str());
#endif
	}
	State.Load();
#if _DEBUG
